    ...
    */
    public void exit(int token){
        if(tokens.containsKey(token)) {
            ...
            sendToEvery("===\nПользователь " + login + " выешел", "USERS");
        ...
        while(true) {
            ...
            tokens.forEach((k, v) -> {
                if (deleteToken(v)) {
                    ...
                    numberOfUsers.decrementAndGet();
                    sendDisconnect(v.port, v.address, "===\nВы отключены от сервера", "DISCONNECTION");
                    tokens.remove(k);
                }
            });
//        Iterator<ConcurrentHashMap.Entry<Integer, SubToken>> iterator = tokens.entrySet().iterator();
//        while(iterator.hasNext()){
//            ConcurrentHashMap.Entry<Integer, SubToken> entry = iterator.next();
//            if(deleteToken(entry.getValue())){
//                logins.append(entry.getValue().login).append(", ");
//                iterator.remove();
//            }
//        }
            if (logins.toString().length() > 0) {
                sendToEvery("===\nВышедшие пользователи: " + logins.toString(), "USERS");
            }
        }
    }

    public void sendToEvery(String message, String specialWord){
    ...
    private void sendDisconnect(Integer port, InetAddress address, String message, String specialWord){
        ByteArrayOutputStream bytte = new ByteArrayOutputStream();
        try (ObjectOutputStream sendstream = new ObjectOutputStream(bytte)) {
            sendstream.writeObject(new ServerMessage(message, specialWord));
            sendstream.flush();
